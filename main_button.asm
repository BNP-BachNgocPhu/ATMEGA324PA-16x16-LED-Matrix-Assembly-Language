.EQU PORT_KEYPAD = PORTB   ; Define the keypad port
.EQU PORT_KEYPAD_DR = DDRB ; Define the keypad data direction register
.EQU INPORT_KEYPAD = PINB  ; Define the keypad input port
.DEF VALUE = R24           ; Define register R24 as VALUE
...
MAIN:
    ...
    OUT SPH,R16            ; Store it in the stack pointer high register (SPH)
    LDI R16,LOW(RAMEND)    ; Load the low byte of RAMEND into R16
    OUT SPL,R16            ; Store it in the stack pointer low register (SPL)

    LDI R16,0B00000111     ; Set the first 3 bits of PORTB as output, others as input
    OUT PORT_KEYPAD_DR,R16 

    LDI R16,$FF
    OUT PORT_KEYPAD,R16    ; Set all bits of the keypad port to high

    LDI R16,$FF
    OUT DDRA,R16           ; Set all bits of PORTA as output

    CBI PORTA,7            ; Disable 573_COL
    CBI PORTA,6            ; Disable 573_ROW

START:
    RCALL SETUP_MATRIX      ; Call the matrix setup routine

WAIT:  
    LDI R16,$FF
    OUT PORT_KEYPAD,R16
    LDI R17,$3B            ; 0x00111011 Scan the first column
    LDI R18,0              ; First row value
    LDI R19,2              ; Column scan value
    RCALL SCANKEY          ; Call the key scan subroutine
    RCALL DELAY_500MS      ; Call the 500ms delay subroutine
    CPI VALUE,0
    BREQ OPTION_KEY_Z
    CPI VALUE,2
    BREQ OPTION_KEY_C
